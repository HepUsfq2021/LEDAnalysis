...
// ----------member data ---------------------------
  ...
  float value_el_HcalOverEcal[max_el];
...
LEDSkimmer::LEDSkimmer(const edm::ParameterSet& iConfig)
 ...
 // Electrons
  ...
  tree->Branch("Electron_HcalOverEcal", value_el_HcalOverEcal, "Electron_HcalOverEcal[nElectron]/F");
...
LEDSkimmer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (auto it = electrons->begin(); it != electrons->end(); it++) {
    if (it->pt() > el_min_pt) {
      ...
      value_el_HcalOverEcal[value_el_n] = it->hcalOverEcal();
